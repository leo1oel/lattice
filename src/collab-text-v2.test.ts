...
describe("v2 durable text client", () => {
  ...
  it("protects main, secondary, draft, and outbox entries, then re-evicts after ACK", async () => {
    const a = await setup(); const b = await setup(new IDBFactory(), { ...namespace, fileId: "b" }); const pool = new CollabTextProviderPoolV2(1); pool.add(a.client); pool.pin(a.client, "main"); pool.pin(a.client, "secondary"); pool.add(b.client); pool.setDraft(b.client, true); expect(pool.size).toBe(2); pool.unpin(a.client, "main"); pool.unpin(a.client, "secondary"); pool.setDraft(b.client, false); expect(pool.size).toBe(2); // Neither unacknowledged remote-open entry may masquerade as clean.
