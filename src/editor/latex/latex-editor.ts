...
function unwrapLatexPath(raw: string): string {
...
function graphicsHoverTarget(text: string, position: number): { from: number; to: number; path: string } | null {
  COMPLETE_GRAPHICS.lastIndex = 0;
  for (let match = COMPLETE_GRAPHICS.exec(text); match; match = COMPLETE_GRAPHICS.exec(text)) {
    const from = match.index + match[0].indexOf("{") + 1;
    const rawPath = match[1].trim();
...
function labelHoverTarget(text: string, position: number): { from: number; to: number; label: string } | null {
