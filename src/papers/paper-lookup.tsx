    ...
    <main className="paper-lookup">
      <header className="paper-lookup-titlebar" data-tauri-drag-region>
        ...
        <button className="icon-button" aria-label={t`Keep on top`} title={t`Keep on top`} aria-pressed={pinned} disabled={pinBusy} onClick={() => void togglePin()}>
          <IconSwap swapKey={pinned ? "pinned" : "unpinned"}>
            {pinned ? <Pin size={14} /> : <PinOff size={14} />}
          </IconSwap>
        </button>
      </header>
      <div className="paper-lookup-search">
        <div className="paper-lookup-project"><span>{library?.projectRoot.split(/[\\/]/).pop() || t`Papers`}</span></div>
      ...
      <div className="paper-lookup-list" role="list" aria-label={t`Papers`}>
        {papers.map((paper) => (
          <div role="listitem" key={`${paper.arxivId}:${paper.citationKey}`} className="paper-lookup-row" draggable onDragStart={(event) => beginPaperDrag(event.dataTransfer, library!.projectRoot, paper, owner)}>
