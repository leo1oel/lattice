...
function inlineMathSourceRaw(source: string, start: number, end: number, value: string): string | null {
  ...
  // the parsed formula; never guess from the formula text alone.
  const lower = Math.max(0, start - 8);
  const upper = Math.min(source.length, end + 8);
  for (let open = lower; open < upper; open += 1) {
    if (source[open] !== '$' || source[open - 1] === '$' || source[open + 1] === '$') continue;
    let close = open + 1;
    while (close < upper) {
      if (source[close] === '$' && source[close - 1] !== '\\') break;
      close += 1;
    }
    if (close >= upper) continue;
    const body = source.slice(open + 1, close);
    const decoded = body.replace(/\\(.)/g, (match, char: string) => (
      ESCAPABLE_CHARS.has(char) ? char : match
    ));
    if (decoded === value) return source.slice(open, close + 1);
  }
  return null;
}
...
export function applyPositionSliceToNode(
  ...
  switch (node.type) {
    ...
    case 'inlineMath': {
      const sourceRaw = inlineMathSourceRaw(source, startOff, endOff, node.value ?? '');
      if (sourceRaw) node.data.sourceRaw = sourceRaw;
      if (sourceRaw?.[0] === '$') {
        let count = 0;
        while (count < sourceRaw.length && sourceRaw[count] === '$') {
          count++;
        }
    ...
    case 'mdxTextExpression': {
      ...
      if (node.type === 'mdxJsxFlowElement' || node.type === 'mdxJsxTextElement') {
        const name = (node as typeof node & { name?: string | null }).name;
        const initial = source.slice(startOff, endOff);
        if (name && !initial.includes(`</${name}>`) && !/\/\s*>$/.test(initial)) {
          const closing = source.indexOf(`</${name}>`, endOff);
          if (closing >= 0) rawEnd = closing + name.length + 3;
        }
      }
      node.data.sourceRaw = source.slice(startOff, rawEnd);
      break;
    }
