...
function buildMdastToPmHandlers(
      ...
      return n.codeBlock.createAndFill(
        {
          ...
          sourceUnclosedFence: node.data?.sourceUnclosedFence ?? false,
  ...
  if (n.mathInline) {
    handlers.inlineMath = (node: {
      type: 'inlineMath';
      value?: string;
      data?: { sourceDelimiter?: string; sourceRaw?: string };
    }) =>
      n.mathInline.create({
        formula: node.value ?? '',
...
function buildPmToMdastHandlers(
    ...
    nodeHandlers.codeBlock = (pmNode: PmNode) => {
      ...
      return {
        ...
        data: {
          ...
          sourceUnclosedFence: pmNode.attrs.sourceUnclosedFence === true ? true : undefined,
  ...
  if (n.jsxComponent) {
    nodeHandlers.jsxComponent = (pmNode: PmNode, _parent: PmNode | undefined, state) => {
      ...
      if (
        (!effectiveDirty(pmNode, freshness.checker) || meta.name === '*')
        && pmNode.attrs.sourceRaw
      ) {
        return {
  ...
  if (n.mathInline) {
    nodeHandlers.mathInline = (pmNode: PmNode) => {
      ...
      const sourceRaw = pmNode.attrs.sourceRaw;
      return {
        ...
        ...(typeof sourceDelimiter === 'string' && sourceDelimiter.length > 0
          ...
          ? { data: {
              ...(typeof sourceDelimiter === 'string' && sourceDelimiter.length > 0 ? { sourceDelimiter } : {}),
              ...(typeof sourceRaw === 'string' && sourceRaw.length > 0 ? { sourceRaw } : {}),
            } }
          : {}),
      } as unknown as MdastNodes;
    };
