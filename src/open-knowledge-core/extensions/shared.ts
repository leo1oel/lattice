...
export const sharedExtensions = [
  ...
  TableFidelity.configure({
    resizable: true,
    // The block grip deliberately creates a NodeSelection on the whole
    // top-level table. TipTap otherwise normalizes that selection into a
    // CellSelection, so Delete clears cell contents instead of removing the
    // selected block like it does for paragraphs, code, and components.
