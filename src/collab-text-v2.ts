...
export class CollabTextClientV2 {
  ...
  private async handleCustomMessage(raw: unknown): Promise<void> { let value = raw; if (typeof raw === "string") { try { value = JSON.parse(raw); } catch { return; } } if (!isDurableAckV2(value) || !this.validAck(value)) return; const ack = value as DurableAckV2; const vector = safeDecodeBase64Url(ack.stateVector); if (!vector) return;
    ...
    await this.store.recordAck(this.namespace, ack); this.revision = ack.contentRevision; this.generation = ack.snapshotGeneration; this.durableSeen = true; this.outbox = await this.store.deleteCovered(this.namespace, vector); this.emitState();
  }
