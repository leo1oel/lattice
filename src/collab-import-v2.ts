...
async function run(options: ImportV2Options): Promise<ImportProjectRecordV2> {
  ...
  try {
    ...
    const files = await buildFiles(options.source, fileIds); fileIds = Object.fromEntries(files.map((file) => [file.path, file.fileId]));
    ...
    for (let offset = 0; offset < textFiles.length; offset += IMPORT_CONCURRENCY) {
      const results = await Promise.allSettled(textFiles.slice(offset, offset + IMPORT_CONCURRENCY).map(async (file) => {
        ...
        await putText(fetcher, options.deployment, id, secret, file, operationId);
...
async function buildFiles(source: NativeImportSourceV2, resumedIds: Record<string, string> = {}): Promise<ImportFileV2[]> {
  ...
  for (let offset = 0; offset < entries.length; offset += IMPORT_CONCURRENCY) {
    out.push(...await Promise.all(entries.slice(offset, offset + IMPORT_CONCURRENCY).map(async (item) => { const bytes = await source.read(item.path); const kind = item.kind ?? (isUtf8(bytes) ? "text" : "binary"); return { fileId: resumedIds[item.path] ?? await stableFileId(item.path), path: item.path, kind: kind === "text" && item.path.toLocaleLowerCase("en-US").endsWith(".tldr") ? "board" as const : kind, size: bytes.length, hash: await sha(bytes), bytes, contentType: kind === "binary" ? "application/octet-stream" : undefined }; })));
  }
...
async function putText(f: typeof fetch,d:string,id:string,s:string,x:ImportFileV2,op:string){ const r=await f(projectUrl(d,id,`text/imports/${encodeURIComponent(x.fileId)}`),{method:"PUT",headers:{...auth(s),"x-document-epoch":"1","x-content-sha256":x.hash,"x-operation-id":op},body:x.bytes});if(!r.ok)throw new Error("text_import_failed"); }
