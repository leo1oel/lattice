...
type ImportResumeV2 = { projectInstanceId: string; credentialRef: string; manifestHash: string; operationId: string; diagnosticOperationId?: string; fileIds: Record<string, string>; completed: string[]; error: string };
class CollabImportV2Error extends Error { constructor(message: string, readonly resume: ImportResumeV2, cause?: unknown) { super(message, { cause }); } }
export type ImportV2Options = { deployment: string; projectName?: string; source: NativeImportSourceV2; credentialStore: CollabCredentialStore; fetch?: typeof fetch; projectInstanceId?: string; idFactory?: () => string; diagnosticOperation?: DiagnosticOperationContext; resume?: ImportResumeV2; policy?: CollabFeaturePolicy; onPhase?: (phase: "importing") => void; onPrepareProgress?: (completed: number, total: number) => void; onProgress?: (completed: number, total: number) => void; onRecord: (record: ImportProjectRecordV2) => Promise<void> };

class TextImportErrorV2 extends Error {
...
async function canonicalImportManifestHash(entries: Omit<ImportFileV2, "bytes" | "contentType">[]): Promise<string> { return sha(new TextEncoder().encode(canonicalJson(entries))); }
...
async function run(options: ImportV2Options): Promise<ImportProjectRecordV2> {
  ...
  const diagnostic = options.diagnosticOperation ?? { operationId: options.resume?.diagnosticOperationId ?? crypto.randomUUID() };
  try {
    ...
    let catalog = await getCatalog(fetcher, options.deployment, id, secret, diagnostic).catch((error) => {
      // Only a missing project permits bootstrap. A service/quota/network
      // failure is not evidence that it is safe to create another project.
      if (error instanceof CollabControlErrorV2 && error.status === 404) return undefined;
      throw error;
    });
    if (!catalog) {
      const response = await jsonFetch(fetcher, projectUrl(options.deployment, id, "bootstrap"), { projectInstanceId: id, projectName: options.projectName, hostSecret: secret, importManifest: plain, expectedManifestHash: manifestHash, operationId: importOperationId }, undefined, diagnostic);
      if (!response.ok) {
    ...
    for (let offset = 0; offset < textFiles.length; offset += IMPORT_CONCURRENCY) {
      const results = await Promise.allSettled(textFiles.slice(offset, offset + IMPORT_CONCURRENCY).map(async (file) => {
        ...
        await putTextFileV2({ fetch: fetcher, deployment: options.deployment, projectInstanceId: id, credential: secret, fileId: file.fileId, documentEpoch: 1, bytes: file.bytes, hash: file.hash, operationId, diagnosticOperation: diagnostic });
        completed.push(file.fileId); options.onProgress?.(++progress, files.length);
      }));
      const failed = results.find((result) => result.status === "rejected");
      if (failed?.status === "rejected") throw failed.reason;
    }
    if (textFiles.length) catalog = await getCatalog(fetcher, options.deployment, id, secret, diagnostic);
    ...
    for (let offset = 0; offset < binaryFiles.length; offset += IMPORT_CONCURRENCY) {
      const results = await Promise.allSettled(binaryFiles.slice(offset, offset + IMPORT_CONCURRENCY).map(async (file) => {
        const operationId = `import_${file.fileId}`.replace(/[^A-Za-z0-9_-]/g, "_");
        await putBinary(fetcher, options.deployment, id, secret, file, catalog!.catalogRevision, operationId, diagnostic);
        completed.push(file.fileId); options.onProgress?.(++progress, files.length);
      }));
      const failed = results.find((result) => result.status === "rejected");
      if (failed?.status === "rejected") throw failed.reason;
    }
    if (binaryFiles.length) catalog = await getCatalog(fetcher, options.deployment, id, secret, diagnostic);
    verifyCatalog(catalog, plain); const finalized = await jsonFetch(fetcher, projectUrl(options.deployment, id, "import-finalize"), { operationId: `finalize_${importOperationId}`, expectedCatalogRevision: catalog.catalogRevision, importOperationId, expectedManifestHash: manifestHash }, secret, diagnostic); if (!finalized.ok) throw new Error("v2_finalize_failed");
    catalog = await getCatalog(fetcher, options.deployment, id, secret, diagnostic); if (catalog.lifecycle !== "live") throw new Error("v2_not_live");
...
export async function putTextFileV2(options: { fetch?: typeof fetch; deployment: string; projectInstanceId: string; credential: string; fileId: string; documentEpoch: number; bytes: Uint8Array; hash?: string; operationId: string; diagnosticOperation?: DiagnosticOperationContext }): Promise<{ size: number; hash: string }> {
  const fetcher = options.fetch ?? globalThis.fetch.bind(globalThis);
  const hash = options.hash ?? await sha(options.bytes);
  const response = await diagnosticFetch(fetcher, projectUrl(options.deployment, options.projectInstanceId, `text/imports/${encodeURIComponent(options.fileId)}`), {
    method: "PUT",
    ...
    body: options.bytes,
  }, "collab.text.import", options.diagnosticOperation);
  if (!response.ok) {
...
async function putBinary(f:typeof fetch,d:string,id:string,s:string,x:ImportFileV2,rev:number,op:string,diagnostic:DiagnosticOperationContext){const t=await jsonFetch(f,projectUrl(d,id,"binary/upload-tickets"),{fileId:x.fileId,documentEpoch:1,declaredHash:x.hash,declaredSize:x.size,contentType:x.contentType,expectedCatalogRevision:rev,expectedContentRevision:0,operationId:op,import:true},s,diagnostic);if(!t.ok)throw new Error("binary_ticket_failed");const ticket=(await t.json() as {ticket:string}).ticket;const u=await diagnosticFetch(f,projectUrl(d,id,`binary/uploads/${encodeURIComponent(ticket)}`),{method:"PUT",headers:{"content-type":x.contentType!},body:x.bytes},"collab.binary.import",diagnostic);if(!u.ok)throw new Error("binary_import_failed");const c=await jsonFetch(f,projectUrl(d,id,"binary/commit"),{ticket,operationId:op},s,diagnostic);if(!c.ok)throw new Error("binary_commit_failed");}
function projectUrl(d:string,id:string,p:string){return `${d.replace(/\/$/,"")}/v2/projects/${encodeURIComponent(id)}/${p}`;} function auth(s:string){return {Authorization:`Bearer ${s}`};} async function jsonFetch(f:typeof fetch,u:string,b:object,s?:string,diagnostic?:DiagnosticOperationContext){return diagnosticFetch(f,u,{method:"POST",headers:{"content-type":"application/json",...(s?auth(s):{})},body:JSON.stringify(b)},"collab.import.control",diagnostic);} async function getCatalog(f:typeof fetch,d:string,id:string,s:string,diagnostic?:DiagnosticOperationContext){const r=await diagnosticFetch(f,projectUrl(d,id,"catalog"),{headers:auth(s)},"collab.catalog",diagnostic);if(!r.ok)throw new CollabControlErrorV2(r.status, await r.json().catch(() => ({})));return r.json() as Promise<CatalogV2>;} function safePath(p:string){p=p.normalize("NFC");if(!p||p.startsWith("/")||p.endsWith("/")||p.includes("\\")||p.split("/").some(x=>!x||x==="."||x===".."))throw new Error("unsafe_path");return p;} function isUtf8(b:Uint8Array){try{new TextDecoder("utf-8",{fatal:true}).decode(b);return true;}catch{return false;}} async function sha(b:Uint8Array){return Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",b)),x=>x.toString(16).padStart(2,"0")).join("");} function canonicalJson(v:unknown):string{if(Array.isArray(v))return`[${v.map(canonicalJson).join(",")}]`;if(v&&typeof v==="object")return`{${Object.entries(v as Record<string,unknown>).sort(([a],[b])=>a.localeCompare(b)).map(([k,x])=>`${JSON.stringify(k)}:${canonicalJson(x)}`).join(",")}}`;return JSON.stringify(v);} async function stableFileId(p:string){return `file_${(await sha(new TextEncoder().encode(p))).slice(0,32)}`;} function randomId(){return `project_${crypto.randomUUID().replaceAll("-","")}`;} function randomSecret(){const b=crypto.getRandomValues(new Uint8Array(32));return btoa(String.fromCharCode(...b)).replaceAll("+","-").replaceAll("/","_").replaceAll("=","");}
