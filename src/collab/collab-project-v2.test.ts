...
describe("v2 project presence", () => {
  ...
  it("announces identity and path on awareness, merges cross-file presence, and leaves on destroy", async () => {
    ...
    const { peerColorForKey } = await import("../components/ui/collab-colors");
  ...
  it("materializes local and remote structured spreadsheet edits instead of the empty content text", async () => {
    ...
    await controller.openPath("data.lattice-sheet");
    const { applySpreadsheetBatch } = await import("../editor/spreadsheet/spreadsheet-operations");
    const { seedSpreadsheetDoc } = await import("../editor/spreadsheet/spreadsheet-yjs");
