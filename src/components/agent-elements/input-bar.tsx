...
export type InputBarProps = {
  ...
  /** Optional host-owned ref and event hooks for command/mention completion. */
  ...
  /** Return true when the host handled the key and default input handling should stop. */
...
export const InputBar = memo(function InputBar({
  ...
  const handleSubmit = useCallback(() => {
    ...
    setInput("");
  }, [attachedFiles.length, attachedImages.length, input, isStreaming, disabled, onSend, setInput]);
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      if (onTextareaKeyDown?.(e) || e.defaultPrevented) return;
            ...
            <div className="flex items-center justify-between gap-2 px-1.5 pt-1 pb-1.5">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  disabled={isStreaming ? disabled : (!hasInput && !hasContextItems) || disabled}
                  onClick={() => {
                    if (isStreaming) {
                      onStop();
                    } else if (hasInput || hasContextItems) {
                      handleSubmit();
                    }
                  }}
                  className="cursor-pointer disabled:cursor-default disabled:opacity-55"
                >
                  <SendButton
                    state={
                      isStreaming
                        ? "streaming"
                        : (hasInput || hasContextItems) && !disabled
                          ? "typing"
                          : "idle"
                    }
                  />
