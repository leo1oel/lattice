...
export const STRUCTURED_MARKDOWN_PROTOTYPE_ONLY = true;
...
export function readStructuredJson(fragment: Y.XmlFragment): JSONContent {
...
/** Visual transactions edit the same fragment; there is no Y.Text mirror. */
export function applyVisualTransaction(fragment: Y.XmlFragment, mutate: (json: JSONContent) => JSONContent) {
  replaceStructuredJson(fragment, mutate(readStructuredJson(fragment)));
}
