...
export type FocusPullProps = {
  blur?: number;
  /**
   * Peak exposure lift during the pull. Local edit to the remocn component:
   * the stock 1.3 reads as a cinematic flare over dark footage, but both of
   * our scenes are white app windows, so it blew the whole frame out to paper
   * white at every cut. Kept as a prop rather than hardcoded so the original
   * look is still one argument away.
   */
  bloom?: number;
};
...
> = ({
  ...
  const { blur = 16, bloom = 1.06 } = passedProps;
  ...
  if (!entering) {
    const exitStyle: React.CSSProperties = {
      ...
      })}px) brightness(${interpolate(p, [0.1, 0.55], [1, bloom], clampOpts)})`,
    };
  ...
  const childStyle: React.CSSProperties = {
    ...
    })}px) brightness(${interpolate(p, [0.4, 0.85], [bloom, 1], clampOpts)})`,
  };
